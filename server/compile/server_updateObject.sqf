...
*/
private ["_object","_type","_objectID","_uid","_lastUpdate","_needUpdate","_object_position","_object_inventory","_object_damage","_isNotOk","_parachuteWest","_firstTime","_object_killed","_object_repair","_isbuildable","_garage"];
...
if ((typeOf _object) in VGGarage) then {_garage = _this select 2;};
...
_object_plotfriends = {
	private ["_plotfriends","_key"];
	_plotfriends = _object getVariable ["plotfriends",[]];
	if (_objectID == "0") then {
		_key = format["CHILD:309:%1:%2:",_uid,_plotfriends];
	} else {
		_key = format["CHILD:303:%1:%2:",_objectID,_plotfriends];
	};
	[_key,""+server_var+"HIVE"] call server_hiveWrite;
};

_object_garage = {
	private ["_garagelist","_key"];
	_garagelist = _this;
...
_object_garagefriends = {
	private ["_damage","_friends","_key"];
	_damage = damage _object;
	_friends = _object getVariable ["GarageFriends",[]];
	if (_objectID == "0") then {
		_key = format["CHILD:306:%1:%2:%3:",_uid,_friends,_damage];
	} else {
		_key = format["CHILD:306:%1:%2:%3:",_objectID,_friends,_damage];
	};
	[_key,""+server_var+"HIVE"] call server_hiveWrite;
};
...
switch (_type) do {
	...
	case "plot": {
		call _object_plotfriends;
	};
	case "garage": {
		_garage call _object_garage;
	};
	case "garagefriends": {
		call _object_garagefriends;
	};
};