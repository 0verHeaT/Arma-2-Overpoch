private ["_mission","_aipack","_class","_position2","_direction","_static","_position","_unitnumber","_skill","_gun","_mags","_backpack","_skin","_gear","_aiweapon","_aigear","_aiskin","_skillarray","_unitGroup","_weapon","_magazine","_weaponandmag","_gearmagazines","_geartools","_unit","_result"];
...
_unit addMPEventHandler ["MPHit", {_this call fnc_plyrHit;}];
_unit addMPEventHandler ["MPKilled", {_this spawn fnc_playerDied;}];
...
if (ai_static_useweapon) then {
	...
	_weaponandmag = _aiStaticweapon call BIS_fnc_selectRandom;
	...
	*/
	...
	ai_bpitems = [ai_bp0,ai_bp1,ai_bp2,ai_bp3,ai_bp4,ai_bp5,ai_bp6] call BIS_fnc_selectRandom;
	{(unitBackpack _unit) addMagazineCargoGlobal [_x,round random 1]} count ai_bpitems;
};
if (ai_static_skills) then {
	if (_class == "TOW_TriPod_US_EP1") then {
		{_unit setSkill [_x,1.00]} forEach _skillarray;
	} else {
